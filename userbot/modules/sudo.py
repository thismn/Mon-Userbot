...
async def sudo(event):
    ...
    if sudo == "True":
        await edit_or_reply(
            event,
            f"🔮 **Sudo:** `Enabled`\n\n**__List Sudo Users:__**\n» `{users}`\n\n**SUDO HANDLER:** `{SUDO_HANDLER}`",
        )
    else:
        await edit_delete(event, "🔮 **Sudo:** `Disabled`")


@my_cmd(pattern="addsudo(?:\\s|$)([\\s\\S]*)")
async def add(event):
    suu = event.text[9:]
    ...
    if not suu and not reply:
        return await edit_delete(
            xxnx,
            "Balas ke pengguna atau berikan user id untuk menambahkannya ke daftar pengguna sudo.",
            45,
        )
...
async def _(event):
    ...
    if gett in sudousers:
        ...
        await edit_delete(
            xxx, "**Pengguna ini tidak ada dalam Daftar Pengguna Sudo.**", 45
        )
