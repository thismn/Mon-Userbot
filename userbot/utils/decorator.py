    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            my_reg = sudo_reg = re.compile(pattern)
        else:
            my_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            my_reg = re.compile(my_ + pattern)
        ...
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(
                    **args, outgoing=True, pattern=my_reg))
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=my_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
